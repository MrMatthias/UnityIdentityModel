...
namespace IdentityModel.Client
{
	...
	public class DiscoveryDocumentResponse : ProtocolResponse
	{
		...
		public string? BackchannelAuthenticationEndpoint =>
			TryGetString(OidcConstants.Discovery.BackchannelAuthenticationEndpoint);
		...
		public string? PushedAuthorizationRequestEndpoint =>
			TryGetString(OidcConstants.Discovery.PushedAuthorizationRequestEndpoint);
		...
		public bool? FrontChannelLogoutSessionSupported =>
			TryGetBoolean(OidcConstants.Discovery.FrontChannelLogoutSessionSupported);

		public IEnumerable<string> GrantTypesSupported =>
			TryGetStringArray(OidcConstants.Discovery.GrantTypesSupported);

		public IEnumerable<string> CodeChallengeMethodsSupported =>
			TryGetStringArray(OidcConstants.Discovery.CodeChallengeMethodsSupported);
		...
		public IEnumerable<string> SubjectTypesSupported =>
			TryGetStringArray(OidcConstants.Discovery.SubjectTypesSupported);

		public IEnumerable<string> ResponseModesSupported =>
			TryGetStringArray(OidcConstants.Discovery.ResponseModesSupported);

		public IEnumerable<string> ResponseTypesSupported =>
			TryGetStringArray(OidcConstants.Discovery.ResponseTypesSupported);

		public IEnumerable<string> ClaimsSupported => TryGetStringArray(OidcConstants.Discovery.ClaimsSupported);

		public IEnumerable<string> TokenEndpointAuthenticationMethodsSupported =>
			TryGetStringArray(OidcConstants.Discovery.TokenEndpointAuthenticationMethodsSupported);

		public IEnumerable<string> BackchannelTokenDeliveryModesSupported =>
			TryGetStringArray(OidcConstants.Discovery.BackchannelTokenDeliveryModesSupported);

		public bool? BackchannelUserCodeParameterSupported =>
			TryGetBoolean(OidcConstants.Discovery.BackchannelUserCodeParameterSupported);

		public bool? RequirePushedAuthorizationRequests =>
			TryGetBoolean(OidcConstants.Discovery.RequirePushedAuthorizationRequests);
		...
		public IEnumerable<string> TryGetStringArray(string name) =>
			Json?.TryGetStringArray(name) ?? Array.Empty<string>();
		...
		private string Validate(DiscoveryPolicy policy)
		{
			if (policy.ValidateIssuerName)
			{
				...
				AuthorityValidationResult issuerValidationResult =
					strategy.IsIssuerNameValid(Issuer!, policy.Authority);
		...
		{
			return new StringComparisonAuthorityValidationStrategy(nameComparison).IsIssuerNameValid(issuer, authority)
				.Success;
		}
		...
		/// <returns></returns>
		private bool ValidateIssuerName(string issuer, string authority,
			IAuthorityValidationStrategy validationStrategy)
		{
		...
		/// <returns></returns>
		public string ValidateEndpoints(JToken? json, DiscoveryPolicy policy)
		{
			if (json == null)
			{
			...
			// allowed hosts
			var allowedHosts =
				new HashSet<string>(policy.AdditionalEndpointBaseAddresses.Select(e => new Uri(e).Authority))
				{
			...
			foreach (var element in json as JObject)
			{
				    ...
				    element.Key.Equals(OidcConstants.Discovery.CheckSessionIframe, StringComparison.OrdinalIgnoreCase))
				{
					...
					if (policy.ValidateEndpoints)
					{
						...
						IAuthorityValidationStrategy strategy = policy.AuthorityValidationStrategy ??
						                                        DiscoveryPolicy.DefaultAuthorityValidationStrategy;
						AuthorityValidationResult endpointValidationResult =
							strategy.IsEndpointValid(endpoint, allowedAuthorities);
